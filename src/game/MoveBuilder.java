...
public class MoveBuilder{
	...
	private static ActualMove moveAlongFile(Move move, Space destination, ErrorMessage message) {
		...
		if(move.getInitialSpace().getPiece().getType() == PieceType.Pawn && rankOffset == 2) // a pawn's first move
		...
		if(initial.getPiece().getType() == PieceType.King && Math.abs(fileOffset) == 2){
			if(fileOffset == 2){ //king side castle
				...
				if(result != null){
					...
					gs.makeMove(rookMove, pieceColor == Color.White ? Turn.Player1 : Turn.Player2, message);
				}
				return result; 
			}
