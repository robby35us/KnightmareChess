...
public class TestUtility {
	...
	public static Board makeBoard(PieceInfo[] info, GameState gs){
		...
		for(int i = 0; i < info.length; i++){
			...
			if(space.getPiece() == null) // verify space is empty
				space.changePiece(factory.makePiece(info[i].getType(), info[i].getColor()));
			else
				return null;
		}
		return board;
	...
	public static void verifyMove(Move move, int rankOffset, int fileOffset, Color color) {
		Assert.assertEquals(color == Color.White ? rankOffset : -rankOffset, move.getRankOffset());
		Assert.assertEquals(color == Color.White ? fileOffset : -fileOffset, move.getFileOffset());
	}
